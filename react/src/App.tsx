...
export class App extends Component<IProps, IState & ISettings> {
    ...
    private getMainContent() {
        ...
        const waiting_players = this.state.players_waiting
            ? this.state.players_waiting.map((player) => player[1])
            : [];
        switch (this.state.game_state) {
            ...
            case "vote": {
                const other_werewolves = this.state.werewolf_names
                    ? this.state.werewolf_names.filter((name) => name !== this.state.nickname)
                    : undefined;
                return (
                    <>
                        <Banner name={this.state.nickname} room={this.state.room_name} />
                        <GameView
                            state={this.state.game_state}
    ...
    render() {
        if (!this.state.token)
            return (
                <Login
                    ...
                    tokenAvailable={this.enterRoom.bind(this)}
                />
            );
            ...
            return (
                <Connection.Provider value={{token: this.state.token}}>
                    {this.state.connected || (
                        <div className="connection-error">No connection to server!</div>
                    )}
                    {this.getMainContent()}
                </Connection.Provider>
            );
        ...
        if (this.state.error) return <div>Server meldet Fehler: {this.state.error}</div>;
    ...
    componentDidUpdate() {
        ...
        if (this.state.game_state === "lobby" || this.state.game_state === "waiting") return;
        if (!this.state.players_waiting) console.error("Missing input: Players waiting in lobby");
        if (this.state.seconds_left === undefined) console.error("Missing input: Countdown status");
        if (this.state.game_state === "choosing") {
            ...
            if (!this.state.seer_name) console.error("Missing input: Seer name");
            if (!this.state.received_votes) console.error("Missing input: Voting results");
        }
    }
    ...
    refresh() {
        ...
        this.timeout_timer = setTimeout(
            (() => this.setState({connected: false})) as TimerHandler,
            5000
        );
            ...
            .then((result) => {
                if (!result.error) {
                    ...
                    this.refresh_timer = setTimeout(
                        this.refresh.bind(this) as TimerHandler,
                        refresh_delay
                    );
                } else {
                    this.setState({is_valid: false, error: result.error});
                    if (result.error === "Invalid token.") this.setState({token: undefined});
                    else setTimeout(this.refresh.bind(this) as TimerHandler, 3000);
                }
            })
