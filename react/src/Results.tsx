...
export class ResultView extends Component<IResultViewProps> {
    render() {
        ...
        const werewolf = werewolf_won ? (
            <div>
                Werwolf{" "}
                <NameGroup items={this.props.werewolf_names} singular="hat" plural="haben" />{" "}
                gewonnen!
            </div>
        ) : (
            <div>
                <NameGroup
                    ...
                    singular="war der Werwolf"
                    plural="waren die Werwölfe"
                />
                .
            </div>
        );
        const seer = (
            <div>
                <b>{this.props.seer_name}</b> war die Seherin.
            </div>
        );

        return (
            <div
                className={"results-container " + (werewolf_won ? "werewolf-won" : "villagers-won")}
            >
        ...
        const list = vote_recipients.map((vote_recipient, index) => {
            ...
            return (
                <li key={index} className="vote-result">
                    <NameGroup items={voters} singular="stimmte" plural="stimmten" /> für{" "}
                    <b>{vote_recipient}</b>
                </li>
            );
        });
