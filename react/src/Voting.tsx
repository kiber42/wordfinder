            ...
            <div className="player-info">
                {this.props.secret_found ? (
                    <div className="secret-message">
                        Das Zauberwort <div className="secret">{this.props.secret}</div> wurde
                        erraten!
                    </div>
                ) : (
                    <div className="secret-message">
                        Ihr habt das Zauberwort <div className="secret">{this.props.secret}</div>{" "}
                        nicht gefunden!
                    </div>
                )}
                {hasVote ? (
                    ...
                    <>
                        <div>
                            <NameGroup
                                items={this.props.werewolf_names}
...
class Vote extends Component<IVoteProps> {
    ...
    render() {
        if (this.props.voted_name)
            return (
                <div>
                    Du hast für <b>{this.props.voted_name}</b> gestimmt. Warte auf andere Spieler.
                </div>
            );
        const choices = this.props.other_players.map((player) => (
            <button onClick={() => this.choose(player[0])} key={player[0]}>
                {player[1]}
            </button>
        ));
    ...
    choose(index: number) {
        fetch("vote.php?token=" + this.context.token + "&id=" + index)
            .then()
